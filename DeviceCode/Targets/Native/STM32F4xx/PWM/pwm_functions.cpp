...
//Timers
static const BYTE g_PWM_Timer[] = PWM_TIMER;
static const BYTE g_PWM_Channel[] = PWM_CHNL;

// Pins
static const BYTE g_PWM_Pins[] = PWM_PINS;
#define PWM_CHANNELS ARRAYSIZE_CONST_EXPR(g_PWM_Pins) // number of channels

// IO addresses
static const ptr_TIM_TypeDef g_PWM_Ports[] =
    {TIM1, TIM2, TIM3, TIM4, TIM5, NULL, NULL, TIM8, TIM9, TIM10, TIM11, TIM12, TIM13, TIM14};
    
// Alternate Function Number
static const BYTE g_PWM_Alt[] = {0x12,0x12,0x22,0x22,0x22,0,0,0x32,0x32,0x32,0x32,0x92,0x92,0x92}; // AF1/2/3/9


//--//

void PWM_UnitializeAll()
{
...
BOOL PWM_Initialize(PWM_CHANNEL channel)
{
    ...
    int timer = g_PWM_Timer[channel];
    int tchnl = g_PWM_Channel[channel];
    ptr_TIM_TypeDef treg = g_PWM_Ports[timer - 1];
    ...
    *reg |= mode;
    ...
    HAL_AddSoftRebootHandler(PWM_UnitializeAll);
...
BOOL PWM_Start(PWM_CHANNEL channel, GPIO_PIN pin)
{
    ...
    CPU_GPIO_DisablePin( pin, RESISTOR_DISABLED, 1, (GPIO_ALT_MODE)g_PWM_Alt[timer - 1] );
...
{
    if ((UINT32)channel >= PWM_CHANNELS) return GPIO_PIN_NONE;
