...
Sockets_FreeRTOS_Driver g_Sockets_FreeRTOS_Driver;

HAL_COMPLETION Sockets_FreeRTOS_Driver::s_DebuggerTimeoutCompletion;
...
{ 
    ...
    return Sockets_FreeRTOS_Driver::Socket( family, type, protocol, FALSE );
}
...
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Connect(socket, address, addressLen);
}
INT32 SOCK_send(SOCK_SOCKET socket, const char* buf, INT32 len, INT32 flags) 
{ 
    NATIVE_PROFILE_PAL_COM();    
    return Sockets_FreeRTOS_Driver::Send(socket, buf, len, flags);
}
INT32 SOCK_recv(SOCK_SOCKET socket, char* buf, INT32 len, INT32 flags)
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Recv(socket, buf, len, flags);
}
INT32 SOCK_close(SOCK_SOCKET socket)
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Close(socket);
}
INT32 SOCK_listen( SOCK_SOCKET socket, INT32 backlog )
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Listen( socket, backlog );
}
SOCK_SOCKET SOCK_accept( SOCK_SOCKET socket, struct SOCK_sockaddr* address, INT32* addressLen )
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Accept( socket, address, addressLen, FALSE );
}
...
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Select( nfds, readfds, writefds, except, timeout );
}
...
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::RecvFrom( s, buf, len, flags, from, fromlen );
}
INT32 SOCK_sendto( SOCK_SOCKET s, const char* buf, INT32 len, INT32 flags, const struct SOCK_sockaddr* to, INT32 tolen )
{ 
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::SendTo( s, buf, len, flags, to, tolen );
}

BOOL Network_Initialize()
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Initialize( );
}
...
BOOL Network_Uninitialize()
{
    ...
    return Sockets_FreeRTOS_Driver::Uninitialize( );
}

void Sockets_FreeRTOS_Driver::CloseDebuggerSocket()
{
    if(g_Sockets_FreeRTOS_Driver.m_SocketDebugStream != SOCK_SOCKET_ERROR)
    {
        if(g_Sockets_FreeRTOS_Driver.m_usingSSL)
        {
            SSL_CloseSocket( g_Sockets_FreeRTOS_Driver.m_SocketDebugStream );
        }
        else
        {
            SOCK_close( g_Sockets_FreeRTOS_Driver.m_SocketDebugStream );
        }

        g_Sockets_FreeRTOS_Driver.m_usingSSL = FALSE;
        
        g_Sockets_FreeRTOS_Driver.m_SocketDebugStream = SOCK_SOCKET_ERROR;

        g_Sockets_FreeRTOS_Driver.m_stateDebugSocket = DbgSock_Listening;

        s_DebuggerTimeoutCompletion.Abort();
    }
}

void Sockets_FreeRTOS_Driver::OnDebuggerTimeout(void* arg)
{
    CloseDebuggerSocket();
}
...
BOOL  SOCKETS_Initialize( int ComPortNum )
{
    ...
    return Sockets_FreeRTOS_Driver::InitializeDbgListener( ComPortNum );
}
BOOL  SOCKETS_Uninitialize( int ComPortNum )
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::UninitializeDbgListener( ComPortNum );
}

INT32 SOCKETS_Write( INT32 ComPortNum, const char* Data, size_t size )
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Write( ComPortNum, Data, size );
}

INT32 SOCKETS_Read( INT32 ComPortNum, char* Data, size_t size )
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::Read( ComPortNum, Data, size );
}
...
{
    NATIVE_PROFILE_PAL_COM();
    Sockets_FreeRTOS_Driver::CloseConnections(FALSE);
}
...
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::UpgradeToSsl( ComPortNum, pCACert, caCertLen, pDeviceCert, deviceCertLen, szTargetHost );
}

BOOL SOCKETS_IsUsingSsl( INT32 ComPortNum )
{
    NATIVE_PROFILE_PAL_COM();
    return Sockets_FreeRTOS_Driver::IsUsingSsl( ComPortNum );
}
...
HRESULT SOCK_CONFIGURATION_UpdateAdapterConfiguration( UINT32 interfaceIndex, UINT32 updateFlags, SOCK_NetworkConfiguration* config )
{
    ...
    if(SUCCEEDED(hr))
    {
        Sockets_FreeRTOS_Driver::SaveConfig(interfaceIndex, config);
    }
    else
    {
...
HRESULT SOCK_CONFIGURATION_LoadWirelessConfiguration( UINT32 interfaceIndex, SOCK_WirelessConfiguration* wirelessConfig )
{
    ...
    Sockets_FreeRTOS_Driver::ApplyWirelessConfig();
...
//
//-----------------------------------------------------------------------------
void Sockets_FreeRTOS_Driver::CloseConnections(BOOL fCloseDbg)
{
    NATIVE_PROFILE_PAL_COM();

    INT32 cnt = g_Sockets_FreeRTOS_Driver.m_cntSockets;
    INT32 idx = 0;

    // round one - close all SSL sockets
    for( INT32 i=cnt-1; i>=0; i-- )
    {
        struct SocketRegisterMap& entry = g_Sockets_FreeRTOS_Driver.m_socketHandles[i];
    ...
    cnt = g_Sockets_FreeRTOS_Driver.m_cntSockets;
    ...
    for( INT32 i=0; i<cnt; i++ )
    {
        ...
        if(entry.m_socket != SOCK_SOCKET_ERROR)
        {
            if(fCloseDbg || !ISSET_SOCKET_FLAG(entry, SocketRegisterMap::c_DebugSocket))
            {
                ...
                g_Sockets_FreeRTOS_Driver.m_socketHandles[i].m_socket  = SOCK_SOCKET_ERROR;
                g_Sockets_FreeRTOS_Driver.m_socketHandles[i].m_flags   = 0;
                g_Sockets_FreeRTOS_Driver.m_socketHandles[i].m_sslData = NULL;
                g_Sockets_FreeRTOS_Driver.m_cntSockets--;
            }
            else if(i > 0)
            {
                memcpy( &g_Sockets_FreeRTOS_Driver.m_socketHandles[i], 
                        &g_Sockets_FreeRTOS_Driver.m_socketHandles[idx++], 
                        sizeof(g_Sockets_FreeRTOS_Driver.m_socketHandles[i]) );
            }
        }
    }
}
...
SOCK_SOCKET Sockets_FreeRTOS_Driver::Socket( INT32 family, INT32 type, INT32 protocol, BOOL fDebug )
{
    NATIVE_PROFILE_PAL_COM();
...
INT32 Sockets_FreeRTOS_Driver::Connect(SOCK_SOCKET socket, const struct SOCK_sockaddr* address, INT32 addressLen)
{
    NATIVE_PROFILE_PAL_COM();

    return HAL_SOCK_connect(socket, address, addressLen);
}
INT32 Sockets_FreeRTOS_Driver::Send(SOCK_SOCKET socket, const char* buf, INT32 len, INT32 flags) 
{
    NATIVE_PROFILE_PAL_COM();

    return HAL_SOCK_send(socket, buf, len, flags);
}
INT32 Sockets_FreeRTOS_Driver::Recv(SOCK_SOCKET socket, char* buf, INT32 len, INT32 flags)
{
    NATIVE_PROFILE_PAL_COM();
...
INT32 Sockets_FreeRTOS_Driver::Shutdown(SOCK_SOCKET sock, INT32 how)
{
    NATIVE_PROFILE_PAL_COM();
...
//
//-----------------------------------------------------------------------------
INT32 Sockets_FreeRTOS_Driver::Close(SOCK_SOCKET sock)
{
    NATIVE_PROFILE_PAL_COM();
    UnregisterSocket(sock);
    return HAL_SOCK_close(sock);
}
INT32 Sockets_FreeRTOS_Driver::Listen( SOCK_SOCKET socket, INT32 backlog )
{
    NATIVE_PROFILE_PAL_COM();

    return HAL_SOCK_listen(socket, backlog);
}
SOCK_SOCKET Sockets_FreeRTOS_Driver::Accept( SOCK_SOCKET socket, struct SOCK_sockaddr* address, INT32* addressLen, BOOL fDebug )
{
    NATIVE_PROFILE_PAL_COM();
...
INT32 Sockets_FreeRTOS_Driver::Select( INT32 nfds, SOCK_fd_set* readfds, SOCK_fd_set* writefds, SOCK_fd_set* exceptfds, const SOCK_timeval* timeout )
{
    NATIVE_PROFILE_PAL_COM();
...
INT32 Sockets_FreeRTOS_Driver::RecvFrom( SOCK_SOCKET s, char* buf, INT32 len, INT32 flags, struct SOCK_sockaddr* from, INT32* fromlen )
{
    NATIVE_PROFILE_PAL_COM();
...
INT32 Sockets_FreeRTOS_Driver::SendTo( SOCK_SOCKET s, const char* buf, INT32 len, INT32 flags, const struct SOCK_sockaddr* to, INT32 tolen )
{
    NATIVE_PROFILE_PAL_COM();
...
void Sockets_FreeRTOS_Driver::MulticastDiscoverySchedule()
{
	if (g_Sockets_FreeRTOS_Driver.m_multicastSocket != SOCK_SOCKET_ERROR)
	{
		...
		privRead.fd_array[0] = g_Sockets_FreeRTOS_Driver.m_multicastSocket;
		privRead.fd_count = 1;
...
void Sockets_FreeRTOS_Driver::MulticastDiscoveryRespond(void* arg)
{
    NATIVE_PROFILE_PAL_COM();
    SOCK_sockaddr from;
    ...
    INT32 len = HAL_SOCK_recvfrom( g_Sockets_FreeRTOS_Driver.m_multicastSocket, data, sizeof(data), 0, &from, &fromlen );
...
BOOL Sockets_FreeRTOS_Driver::InitializeDbgListener( int ComPortNum )
{   
    NATIVE_PROFILE_PAL_COM();
    SOCK_sockaddr_in sockAddr;
    INT32 nonblocking = 1;
    ...
    if(g_Sockets_FreeRTOS_Driver.m_SocketDebugListener != SOCK_SOCKET_ERROR) return TRUE;

    s_DebuggerTimeoutCompletion.InitializeForUserMode(OnDebuggerTimeout);

    //-- debug api socket --//

    g_Sockets_FreeRTOS_Driver.m_SocketDebugListener = Socket( SOCK_AF_INET, SOCK_SOCK_STREAM, SOCK_IPPROTO_TCP, TRUE );
    SOCKET_CHECK_RESULT( g_Sockets_FreeRTOS_Driver.m_SocketDebugListener );
    ...
    SOCKET_CHECK_RESULT( SOCK_ioctl(g_Sockets_FreeRTOS_Driver.m_SocketDebugListener, SOCK_FIONBIO, &nonblocking) );

    SOCKET_CHECK_RESULT( HAL_SOCK_setsockopt( g_Sockets_FreeRTOS_Driver.m_SocketDebugListener, SOCK_IPPROTO_TCP, SOCK_TCP_NODELAY, (char*)&optval, sizeof(optval) ) );

    SOCKET_CHECK_RESULT( HAL_SOCK_setsockopt(g_Sockets_FreeRTOS_Driver.m_SocketDebugListener, SOCK_SOL_SOCKET, SOCK_SOCKO_LINGER, (const char*)&optLinger, sizeof(INT32)) );

    SOCKET_CHECK_RESULT( SOCK_bind( g_Sockets_FreeRTOS_Driver.m_SocketDebugListener,  (SOCK_sockaddr*)&sockAddr, sizeof(sockAddr) ) );

    SOCKET_CHECK_RESULT( SOCK_listen( g_Sockets_FreeRTOS_Driver.m_SocketDebugListener, 1 ) );
    ...
    if(g_Sockets_FreeRTOS_Driver.m_SocketDebugListener != SOCK_SOCKET_ERROR)
    {
        SOCK_close(g_Sockets_FreeRTOS_Driver.m_SocketDebugListener);
        g_Sockets_FreeRTOS_Driver.m_SocketDebugListener = SOCK_SOCKET_ERROR;
...
BOOL Sockets_FreeRTOS_Driver::UninitializeDbgListener( int ComPortNum )
{
    NATIVE_PROFILE_PAL_COM();

    if(ComPortNum != ConvertCOM_SockPort(COM_SOCKET_DBG)) return FALSE;

    g_Sockets_FreeRTOS_Driver.m_stateDebugSocket = DbgSock_Uninitialized;
...
void Sockets_FreeRTOS_Driver::SaveConfig(INT32 index, SOCK_NetworkConfiguration *cfg)
{
    NATIVE_PROFILE_PAL_COM();
...
void Sockets_FreeRTOS_Driver::ApplyConfig()
{
    NATIVE_PROFILE_PAL_COM();
...
void Sockets_FreeRTOS_Driver::ApplyWirelessConfig()
{
    NATIVE_PROFILE_PAL_COM();
...
void Sockets_FreeRTOS_Driver::SaveWirelessConfig(INT32 index, SOCK_NetworkConfiguration *cfg)
{
    NATIVE_PROFILE_PAL_COM();
...
//
//-----------------------------------------------------------------------------
BOOL Sockets_FreeRTOS_Driver::InitializeMulticastDiscovery()
{
    NATIVE_PROFILE_PAL_COM();
    SOCKET_CHECK_ENTER(); 
    SOCK_sockaddr_in sockAddr;
    ...
    if(g_Sockets_FreeRTOS_Driver.s_discoveryInitialized) return TRUE;
    ...
    g_Sockets_FreeRTOS_Driver.m_multicastSocket = Socket( SOCK_AF_INET, SOCK_SOCK_DGRAM, SOCK_IPPROTO_UDP, FALSE );
    SOCKET_CHECK_RESULT( g_Sockets_FreeRTOS_Driver.m_multicastSocket );
    ...
    SOCKET_CHECK_RESULT( SOCK_ioctl(g_Sockets_FreeRTOS_Driver.m_multicastSocket, SOCK_FIONBIO, &nonblocking) );

    SOCKET_CHECK_RESULT( SOCK_setsockopt( g_Sockets_FreeRTOS_Driver.m_multicastSocket, SOCK_IPPROTO_IP, SOCK_IPO_ADD_MEMBERSHIP, (const char*)&multicast, sizeof(multicast) ) );

    SOCKET_CHECK_RESULT( SOCK_bind( g_Sockets_FreeRTOS_Driver.m_multicastSocket, (SOCK_sockaddr*)&sockAddr, sizeof(sockAddr) ) );

    g_Sockets_FreeRTOS_Driver.s_discoveryInitialized = TRUE;
    ...
    if(g_Sockets_FreeRTOS_Driver.m_multicastSocket != SOCK_SOCKET_ERROR)
    {
        SOCK_close(g_Sockets_FreeRTOS_Driver.m_multicastSocket);
        g_Sockets_FreeRTOS_Driver.m_multicastSocket = SOCK_SOCKET_ERROR;
...
BOOL Sockets_FreeRTOS_Driver::Initialize()
{
    NATIVE_PROFILE_PAL_COM();
    SOCKET_CHECK_ENTER(); 
    ...
    if(!s_initialized)
    {
        g_Sockets_FreeRTOS_Driver.m_fShuttingDown = FALSE;
        ...
        g_Sockets_FreeRTOS_Driver.m_cntSockets = 0;
...
BOOL Sockets_FreeRTOS_Driver::Uninitialize( )
{
    NATIVE_PROFILE_PAL_COM();
    BOOL ret = TRUE;
...
//
//-----------------------------------------------------------------------------
INT32 Sockets_FreeRTOS_Driver::Write( int ComPortNum, const char* Data, size_t size )
{
    NATIVE_PROFILE_PAL_COM();
    INT32 ret;
    ...
    if((g_Sockets_FreeRTOS_Driver.m_stateDebugSocket  != DbgSock_Connected) ||
       (g_Sockets_FreeRTOS_Driver.m_SocketDebugStream == SOCK_SOCKET_ERROR)) 
    {
    ...
    if(g_Sockets_FreeRTOS_Driver.m_usingSSL)
    {
        ret = SSL_Write(g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, Data, size);
    }
    else
    {
        ret = SOCK_send( g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, Data, size, 0 );
    }
...
//
//-----------------------------------------------------------------------------
INT32 Sockets_FreeRTOS_Driver::Read( int ComPortNum, char* Data, size_t size )
{
    NATIVE_PROFILE_PAL_COM();
    SOCK_SOCKET sock;
    ...
    if(g_Sockets_FreeRTOS_Driver.m_stateDebugSocket == DbgSock_Uninitialized) return 0;
    ...
    readSet.fd_array[0] = g_Sockets_FreeRTOS_Driver.m_SocketDebugListener;
    readSet.fd_count = 1;
    ...
    if(g_Sockets_FreeRTOS_Driver.m_stateDebugSocket == DbgSock_Connected)
    {
        if(g_Sockets_FreeRTOS_Driver.m_usingSSL)
        {
            ret = SSL_Read(g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, Data, size);
        }
        else
        {
            ret = SOCK_recv(g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, Data, size, 0);
        }
    ...
    if(SOCK_SOCKET_ERROR != HAL_SOCK_select( SOCK_FD_SETSIZE, &readSet, NULL, NULL, &timeout))
    {
        ...
        sock = Accept( g_Sockets_FreeRTOS_Driver.m_SocketDebugListener, (SOCK_sockaddr*)&addr, &len, TRUE );
        ...
        if(SOCK_SOCKET_ERROR != sock)
        {
            ...
            if(DbgSock_Connected == g_Sockets_FreeRTOS_Driver.m_stateDebugSocket)
            {
            ...
            {
                g_Sockets_FreeRTOS_Driver.m_SocketDebugStream = sock;
                SOCKET_CHECK_RESULT( SOCK_ioctl(g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, SOCK_FIONBIO, &nonblocking) );
                SOCKET_CHECK_RESULT( HAL_SOCK_setsockopt( g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, SOCK_IPPROTO_TCP, SOCK_TCP_NODELAY, (char*)&optval, sizeof(optval) ) );
                SOCKET_CHECK_RESULT( HAL_SOCK_setsockopt( g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, SOCK_SOL_SOCKET, SOCK_SOCKO_LINGER, (const char*)&optLinger, sizeof(INT32) ) );

                g_Sockets_FreeRTOS_Driver.m_stateDebugSocket = DbgSock_Connected;
...
BOOL Sockets_FreeRTOS_Driver::IsUsingSsl( int ComPortNum )
{
    if(ComPortNum != ConvertCOM_SockPort(COM_SOCKET_DBG)) return FALSE;
    if(g_Sockets_FreeRTOS_Driver.m_stateDebugSocket == DbgSock_Uninitialized) return FALSE;

    if(g_Sockets_FreeRTOS_Driver.m_stateDebugSocket == DbgSock_Connected)
    {
        return g_Sockets_FreeRTOS_Driver.m_usingSSL;
    }
...
BOOL Sockets_FreeRTOS_Driver::UpgradeToSsl( int ComPortNum, const UINT8* pCACert, UINT32 caCertLen, const UINT8* pDeviceCert, UINT32 deviceCertLen, LPCSTR szTargetHost  )
{
    if(ComPortNum != ConvertCOM_SockPort(COM_SOCKET_DBG)) return 0;
    if(g_Sockets_FreeRTOS_Driver.m_stateDebugSocket == DbgSock_Uninitialized) return 0;
    ...
    if(g_Sockets_FreeRTOS_Driver.m_stateDebugSocket == DbgSock_Connected)
    {
        if(g_Sockets_FreeRTOS_Driver.m_usingSSL) return TRUE;
        ...
        if(SSL_ClientInit( 0x10, 0x04, (const char*)pDeviceCert, deviceCertLen, NULL, g_DebuggerPort_SslCtx_Handle ))
        {
            ...
            do
            {
                ret = SSL_Connect( g_Sockets_FreeRTOS_Driver.m_SocketDebugStream, szTargetHost, g_DebuggerPort_SslCtx_Handle );
            }
            ...
            {
                g_Sockets_FreeRTOS_Driver.m_usingSSL = TRUE;
            }
...
//
//-----------------------------------------------------------------------------
void Sockets_FreeRTOS_Driver::RegisterSocket( SOCK_SOCKET sock, BOOL selectable, BOOL fDebug )
{
    NATIVE_PROFILE_PAL_COM();
    if(sock == SOCK_SOCKET_ERROR)
    {
    ...
    if(g_Sockets_FreeRTOS_Driver.m_cntSockets >= SOCKETS_MAX_COUNT) return;

    GLOBAL_LOCK_SOCKETS(lock);

    g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets].m_socket  = sock;
    g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets].m_flags   = 0;
    g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets].m_sslData = NULL;

    if(fDebug) SET_SOCKET_FLAG(g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets], SocketRegisterMap::c_DebugSocket);
...
//
//-----------------------------------------------------------------------------
void Sockets_FreeRTOS_Driver::UnregisterSocket( SOCK_SOCKET sock )
{
    INT32 index= -1;
    ...
    g_Sockets_FreeRTOS_Driver.GetSocketSslData(sock, index);

    if (index == -1) return;

    g_Sockets_FreeRTOS_Driver.m_cntSockets--;

    if(index != g_Sockets_FreeRTOS_Driver.m_cntSockets)
    {
        memcpy( &g_Sockets_FreeRTOS_Driver.m_socketHandles[index], 
                &g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets], 
                sizeof(g_Sockets_FreeRTOS_Driver.m_socketHandles[index]) );
    }

    g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets].m_socket = SOCK_SOCKET_ERROR;
    g_Sockets_FreeRTOS_Driver.m_socketHandles[g_Sockets_FreeRTOS_Driver.m_cntSockets].m_flags  = 0;
}

BOOL Sockets_FreeRTOS_Driver::s_initialized=FALSE;
BOOL Sockets_FreeRTOS_Driver::s_wirelessInitialized=FALSE;
BOOL Sockets_FreeRTOS_Driver::s_discoveryInitialized=FALSE;
